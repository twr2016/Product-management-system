...
function deleteall(){
	...
	window.location.href="${ctx }/role/deleteall?values="+value;
}
function checkall(){
...
function checkevery(){
	...
	function search(){
		...
		window.location.href="${ctx }/role/list?searchParam="+p;
...
<table class="table">
  ...
  <tbody>
    ...
    <c:forEach items="${page.list }" var="r">
					<tr>
					    <td><input type="checkbox" name="single" value="${r.roleId}" onclick="checkevery()"/></td>
						<td>${r.roleId}</td>
						<td>${r.name }</td>
						
						 <td>
          <a href="${ctx }/role/edit?roleId=${r.roleId }"><i class="fa fa-pencil"></i></a>
          <a href="${ctx }/role/delete?roleId=${r.roleId }" role="button" data-toggle="modal"><i class="fa fa-trash-o"></i></a>
      </td>
					</tr>
				</c:forEach>
  </tbody>
...
<ul class="pagination">
  ...
  <c:forEach begin="1" end="${page.totalPageNum }" var="pageNum">
						<li><a name="pagen" href="${ctx }/role/list?pageNum=${pageNum }">${pageNum }</a></li>
					</c:forEach>
