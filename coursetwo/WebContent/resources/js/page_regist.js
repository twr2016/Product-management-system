$(document).ready(function(){
	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			email: {
				required: $.i18n.prop("请输入注册邮箱"),
				email: $.i18n.prop("请输入正确的邮箱名")
			},
			password: {
				required: $.i18n.prop("请输入密码"),
				minlength: jQuery.format($.i18n.prop("请输入4-16位的密码")),
				maxlength: jQuery.format($.i18n.prop("请输入4-16位的密码"))
			},
			passwordAgain: {
				required: $.i18n.prop("请重新输入密码"),
				minlength: jQuery.format($.i18n.prop("前后输入密码不一致")),
				maxlength: jQuery.format($.i18n.prop("前后输入密码不一致")),
				equalTo: jQuery.format($.i18n.prop("前后输入密码不一致"))
			},
			loginName: {
				required: $.i18n.prop("请输入真实姓名")
			},
			company: {
				required: $.i18n.prop("请输入角色id")
			},
			tel: {
				required: $.i18n.prop("请输入真实姓名"),
